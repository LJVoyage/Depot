...
namespace VoyageForge.Depot.Editor
{
    ...
    public static class AssetsTreeViewAliasPatch
    {
        ...
        static MethodBase TargetMethod()
        {
            ...
            // 获取 FetchData 方法
            var method = dataSourceType.GetMethod("FetchData",
        ...
        static void Postfix(object __instance) // __instance 是 AssetsTreeViewDataSource
        {
            ...
            try
            {
                ...
                var folderTreeItemType =
                    Type.GetType("UnityEditor.AssetsTreeViewDataSource+FolderTreeItem, UnityEditor.CoreModule");
                        ...
                        foreach (var item in rows)
                        {
                            if (item != null && folderTreeItemType.IsAssignableFrom(item.GetType()))
                            {
                                ...
                                if (!string.IsNullOrEmpty(guid) && TestAliases.TryGetValue(guid, out string alias))
                                {
                                    ...
                                    var displayNameField = typeof(TreeViewItem).GetField("displayName",
